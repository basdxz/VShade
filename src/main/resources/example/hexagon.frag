...
#define MOTION_DIVISOR 4.
#define CAMERA_DIVISOR 8.
#define ROUNDING .002
...
float hexPylon(vec2 p2, float pz, float r, float ht){
    ...
    return length(max(abs(p) - b + ROUNDING, 0.)) - ROUNDING;
}
...
float objDist(vec2 p, float pH, float r, float ht, inout float id, float dir){
    ...
    ht = ht - smoothstep(.5, .875, sin(6.283 + iTime/MOTION_DIVISOR)) * ht + 0.2;
    // Neon light height: Four levels, plus the height is divided by two.
    const float s = 1./64.;//1./4./2.*.5;
    ...
    float h2 = hexPylon(p, pH + ht - s, r + .003, s/3.);
...
vec3 envMap(vec3 p){
    ...
    //c = smoothstep(.5, 1.75, c); // Putting in some dark space.
...
vec3 doColor(in vec3 sp, in vec3 rd, in vec3 sn, in vec3 lp, in float t){
    ...
    if (t<FAR){
        ...
        float sz0 = 1./1.;
        vec3 txP = sp;
        sn = texBump(iChannel0, txP/2, sn, .01);///(1. + t/FAR)
        ...
        objCol = mix(objCol, texture(iChannel1, reflect(rd, normalize(sn)).xy).xyz, .015);

        // Combining the above terms to produce the final scene color.
        sceneCol = objCol*(diff + vec3(.3, .3, .6) + .5*ao + vec3(.3, .5, 1)*fres*fres*2.);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // Camera Setup.
    vec3 lk = vec3(0, 1.25, iTime/CAMERA_DIVISOR);// "Look At" position.
